...
namespace my
{
  ...
  namespace detail
  {
    ...
    constexpr auto merge_tuples(std::tuple<T...> d2, TupleArgs tuple_args)
    {
      return d2;
    }

    template <class... T1, class T2, class... T3, class TupleArgs>
    constexpr auto merge_tuples(std::tuple<T1...> d2, TupleArgs tuple_args, T2 head, T3... rest)
    {
      const auto concated = std::tuple_cat(d2, std::make_tuple(head));

      return merge_tuples(concated, tuple_args, rest...);
    }

    template <class... T1, unsigned N, class... T2, class TupleArgs>
    constexpr auto merge_tuples(std::tuple<T1...> d2, TupleArgs tuple_args, placeholders::index<N> head, T2... rest)
    {
    ...
    constexpr auto merge_tuples_imp(std::tuple<T1...> d1, TupleArgs tuple_args, sequence<Ns...>)
    {
      return merge_tuples(std::tuple<>{}, tuple_args, std::get<Ns>(d1)...);
    }

    template <class... T1, class TupleArgs>
    constexpr auto merge_tuples_base(std::tuple<T1...> d1, TupleArgs tuple_args)
    {
    ...
    public:
      ...
      auto operator()(NewArgs... newargs)
      {
        ...
        const auto merged_tuple = merge_tuples_base(args, tuple_args);
